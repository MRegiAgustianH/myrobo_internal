...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div>
        ...
        {{-- MOBILE VIEW INSTRUKTUR --}}
        <div class="space-y-4 md:hidden mb-6">
            @forelse($absensiInstrukturs as $ai)
            <div class="bg-white border border-[#E3EEF0] rounded-xl p-4 shadow-sm text-sm">
                <div class="font-semibold text-gray-800">
                    {{ $ai->instruktur->name }}
                </div>
                <div class="text-xs text-gray-600">
                    {{ $ai->jadwal->sekolah->nama_sekolah ?? 'Home Private' }}
                </div>
                <div class="mt-2 text-xs">
                    <strong>Kegiatan:</strong> {{ $ai->jadwal->nama_kegiatan ?? '-' }}
                </div>
                <div class="text-xs mt-1">
                    {{ $ai->tanggal->format('d/m/Y') }}
                </div>
                <div class="mt-3">
                    <span class="px-2 py-1 rounded-full text-xs font-semibold
                        @if($ai->status === 'hadir') bg-green-100 text-green-700
                        @elseif($ai->status === 'sakit') bg-yellow-100 text-yellow-700
                        @elseif($ai->status === 'izin') bg-blue-100 text-blue-700
        ...
        <div class="hidden md:block bg-white border border-[#E3EEF0] rounded-2xl shadow-sm overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-[#E3EEF0]">
                    @forelse($absensiInstrukturs as $i => $ai)
                    <tr>
                        <td class="px-4 py-2 text-gray-500">{{ $i + 1 }}</td>
                        <td class="px-4 py-2 font-medium text-gray-900">
                            {{ $ai->instruktur->name }}
                            <div class="text-xs text-gray-500 font-normal">
                                {{ $ai->jadwal->nama_kegiatan ?? '-' }}
                            </div>
                        </td>
                        <td class="px-4 py-2">
                            {{ $ai->jadwal->sekolah->nama_sekolah ?? 'Home Private' }}
                        ...
                        <td class="px-4 py-2 text-center">
                            <div class="text-xs mb-1">{{ $ai->tanggal->format('d/m/y') }}</div>
    ...
    <div>
        ...
        <div class="space-y-4 md:hidden">
            ...
            <div class="bg-white border border-[#E3EEF0] rounded-xl p-4 shadow-sm text-sm">
                ...
                <div class="mt-2 text-xs">
                    <strong>Kegiatan:</strong> {{ $a->jadwal?->nama_kegiatan ?? '-' }}
                </div>
                <div class="text-xs mt-1">
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-[#E3EEF0]">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 text-center">
                            <div class="text-xs mb-1">{{ optional($a->tanggal)->format('d/m/y') }}</div>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide
                                @if($a->status === 'hadir') bg-green-100 text-green-700
                                @elseif($a->status === 'sakit') bg-yellow-100 text-yellow-700
                                @elseif($a->status === 'izin') bg-blue-100 text-blue-700
